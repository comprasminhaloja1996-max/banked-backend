...
======================================================== */
app.get('/', (req, res) => {
  res.send('🚀 API COMPLETA - ROTAS ATIVAS!');
});

/* ========================================================
   ROTAS QUE ESTAVAM FALTANDO (ESSENCIAIS)
======================================================== */

// 1. SALDO (Resolve o 404)
...
app.get('/perguntas/:usuario_id/:categoria', async (req, res) => {
  ...
  try {
    const query = `SELECT id, pergunta AS q, json_build_array(opcao_a, opcao_b, opcao_c, opcao_d) AS options, resposta_correta AS answer FROM perguntas WHERE categoria = $1 ORDER BY RANDOM() LIMIT 10`;
